...
void workflow_import_binary_mesh_set( std::string sourceFolder, std::vector<mesh_chunk> ** out_chunks )
{
	std::vector<mesh_chunk> * result = new std::vector<mesh_chunk>( );
	...
	if( partitionSchemeFiles.size( ) > 0 )
	{
		...
		for( std::size_t i = 0; i < mappedChunks.size( ); i++ )
		{
