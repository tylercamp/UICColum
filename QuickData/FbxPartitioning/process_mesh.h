...
void process_mesh( const std::string & file, ExportMode exportMode, bool shouldGenerateCenter = false )
{
	auto start = clock( );
	...
	exportWorkflow( getStoragePath( file ) + "/surfaces", chunks );
	if( volumeChunks ) exportWorkflow( getStoragePath( file ) + "/volumes", volumeChunks );
