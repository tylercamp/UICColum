...
//#define VOLUME_MEMBER_COUNT 12
#define VOLUME_MEMBER_COUNT 4
...
void workflow_gen_volume_normals( gpu_triangle_array * tris, int numVolumes )
{
	auto & mesh = *tris;
	...
	for( int i = 0; i < mesh.extent.size( ); i++ )
	{
		...
		if( volume.numStored >= VOLUME_MEMBER_COUNT )
			__debugbreak( );
